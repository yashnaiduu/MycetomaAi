...
class MultiTaskTrainer:
    ...
    def __init__(self, model, optimizer, train_loader, val_loader, device, epochs=50, save_dir="checkpoints/multitask", checkpoint_every_n_epochs=5, resume_from: Optional[str] = None, grad_accum_steps: int = 1, use_amp: bool = True, early_stop_patience: Optional[int] = None, wandb_run=None):
        self.model = model
    ...
    def _load_checkpoint(self, path: str):
        ...
        logger.info("Resumed finetuning from %s (epoch %s)", path, self.start_epoch)
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if self.wandb_run is not None:
                self.wandb_run.log({
                    ...
                    "train/detect_loss": float(loss_dict.get("detect_loss", 0)),
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if self.early_stop_patience is not None and self.no_improve_epochs >= self.early_stop_patience:
                logger.info("Early stopping triggered after %s epochs with no improvement.", self.no_improve_epochs)
                break